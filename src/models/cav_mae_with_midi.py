...
class Block(nn.Module):
    ...
    def forward(self, x, modality=None):
        if modality == None:
            ...
            x = x + self.drop_path(self.attn(self.norm1_a1(x)))
            x = x + self.drop_path(self.mlp(self.norm2_a1(x)))
        elif modality == "a2":
            x = x + self.drop_path(self.attn(self.norm1_a2(x)))
            x = x + self.drop_path(self.mlp(self.norm2_a2(x)))
        elif modality == "v":
...
class CAVMAE(nn.Module):
    ...
    def forward_contrastive(
        ...
        if audio_rep.nelement() == 0 or video_rep.nelement() == 0:
            ...
            return torch.tensor(0.0, device=audio_rep.device), torch.tensor(
    ...
    def forward(
        ...
        if contrast_loss_weight != 0:
            ...
            loss_c_a1, c_acc_a1 = self.forward_contrastive(
            ...
            loss_c_a2, c_acc_a2 = self.forward_contrastive(
                latent_c_a1.mean(dim=1),
            ...
            loss_c = contrast_loss_weight * (loss_c_a1 + loss_c_a2) / 2
            c_acc = (c_acc_a1 + c_acc_a2) / 2
        else:
